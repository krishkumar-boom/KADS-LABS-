        ...
        <div className="space-y-4 max-h-[700px] overflow-y-auto pr-2">
          {tickets.map((t) => (
            <div key={t.id} className="p-4 rounded-xl bg-white/5 border border-white/10">
              ...
              <p className="text-sm text-white/60 mb-3">{(t as any).message || t.description}</p>
              ...
              {(t as any).replies && (t as any).replies.length > 0 && (
                <div className="space-y-2 mb-3 pl-3 border-l-2 border-electric/30">
                  {(t as any).replies.map((r: any) => (
                    <div key={r.id} className="text-sm p-2 rounded bg-white/5">
                      <p className="text-white/80">{r.text || r.message}</p>
                      <p className="text-xs text-white/40">{r.author || r.author_name} · {new Date(r.createdAt || r.created_at).toLocaleString()}</p>
                    </div>
                  ))}
                </div>
              )}
