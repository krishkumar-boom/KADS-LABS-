...
export default function Careers() {
  ...
  const [form, setForm] = useState({ name: "", email: "", phone: "", resumeUrl: "", coverLetter: "", website: "" })
  ...
  const closeModal = () => {
    ...
    setSubmitted(false)
    setError("")
    setForm({ name: "", email: "", phone: "", resumeUrl: "", coverLetter: "", website: "" })
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true)
    setError("")
    // Primary: leads helper (fires email + writes with lead_id)
    const leadRes = await submitJobApplication({
      name: form.name,
      email: form.email,
      phone: form.phone,
      ...
      source: "careers"
    }).catch(() => ({ ok: false, leadId: undefined as string | undefined })) as { ok: boolean; leadId?: string }
    // Legacy: keep existing system in sync (local demo storage / direct supabase insert)
    ...
    setSubmitting(false)
    if (result.error && !leadRes?.ok) {
      setError(result.error)
      toast.error("Submission failed. Please email us directly.")
    } else {
      setSubmitted(true)
      toast.success(leadRes.leadId ? `Application received! ID: ${leadRes.leadId}` : "Application received! We'll be in touch.")
    }
  }
